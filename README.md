...
> 产品族分组与启动跨期约束下的多周期批量计划问题 (Lot Sizing Problem) 求解器
>
...
| 产品族 | Product Family / Group | 工艺相似的产品集合, 共享启动状态 |
| 流向 | Flow / Downstream | 产品完成后的下游加工路径 |
| 启动 | Setup | 为生产某产品族进行的准备工作 |
...
```
产品族 G1: [产品A, 产品B, 产品C]  -- 共享启动状态
产品族 G2: [产品D, 产品E]        -- 共享启动状态
产品族 G3: [产品F, 产品G, 产品H]  -- 共享启动状态
```
...
| $c^{x}_{i}, s^{x}_{i}$ | `Order` 结构体 | 订单成本、产能消耗 |
| $c^{y}_{g}, s^{y}_{g}$ | `cost_y[]`, `usage_y[]` | 族启动参数 |
| $c^{I}_{f}$ | `cost_i[]` | 流向库存成本 |
...
- [数学模型与算法分析](docs/20260108_1921_LS-NTGF-All_数学模型与算法分析.md) - 完整的数学公式和算法描述
- [RR算法跨期机制问题分析](docs/20260110_1147_RR算法跨期机制问题分析.md) - RR 算法问题诊断与修复方案
