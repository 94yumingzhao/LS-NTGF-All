...
```
说明：约束(6)-(10)管理换型跨期，确保换型可从一个周期延续到下一周期。

**(7) 单一跨期约束：**
```
SUM_{g in G} lambda_gt <= 1,  对所有 t in T
```
说明：每个周期最多只能有一个订单族的换型跨期。
...
利用第一阶段获得的换型配置 y_gt*，求解以下优化问题以最大化换型跨期次数：
...
5. Sox and Gao (1999), Haase (1996) - 换型跨期的影响研究
6. Dillenberger (1994) - 考虑换型跨期的分支定界方法
