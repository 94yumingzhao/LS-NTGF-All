...
| **Setup跨期** | Setup Carryover | 上一周期的setup状态延续到下一周期，减少启动时间和成本 |
...
| y_gt | 二元变量 | 周期 t 是否对订单族 g 进行换型（1=是） |
| I_ft | 连续变量 | 下游工序 f 在周期 t 末的库存水平 |
| lambda_gt | 二元变量 | 订单族 g 的换型是否从 t-1 结转到 t（1=是） |
